...
<div class="carousel">
	...
	<div class="project" role="button" tabindex="0" on:keydown={(e) => (e.key === "Enter" || e.key === " ") && e.target.click()}>
...
{/if}
	...
	<Typewriter mode="cascade" delay="6500" interval="30" on:done={starText=true}>
	<p><span class="text">I hope you like them!</span></p>
	<p><span class="text">These projects are actually updated straight from Github! Go give them a star and see them update here!</span></p>
	</Typewriter>
	{#if showModal}
		<div
			class="modal-overlay"
			...
			<div
				...
				<button on:click={() => showModal = false}
				        on:keydown={(e) => (e.key === "Enter" || e.key === " " || e.key === "esc") && (showModal = false)}><span class="text">Thanks for the star!</span></button>
			</div>
		</div>
	{/if}
</div>
